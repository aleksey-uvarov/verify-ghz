...
import shared
import telescope
from shared import get_ghz_circuit, fidelity_population
from shared import only_h_noise_model


def error_as_nq_experiment(qubit_numbers, n_shots=2**15, noise_model=None):
    """Calculate and plot the errors of all relevant variables (pop, PO coherence, MQC coherence,
    telescopic bounds) as a function pf the number of QUBITS"""
...
def all_errors_experiment(shot_numbers, n_qubits, noise_model=None):
    ...
    Hamiltonian bounds as functions of the number of shots"""
...
def compare_fidelity_estimates(n_qubits=4, noise_model=None):
    shots_coherence = 1e5
    shots_pop = 1e5
    ...
    telescope_data = telescope.fidelity(n_qubits,
                                        ...
                                        noise_model=noise_model)
    telescope_better_data = telescope.ghz_tailored_fidelity(n_qubits,
                                                            shots_coherence + shots_pop,
                                                            noise_model=noise_model)
    ...
    print("Telescope upper (tailored) ", telescope_better_data[1], telescope_better_data[3])
